...
E_VAL = {'A': 1, 'B': 3, 'C': 3, 'D': 2, 'E': 1, 'F': 4, 'G': 2, 'H': 4,
                  'I': 1, 'J': 8, 'K': 5, 'L': 1, 'M': 3, 'N': 1, 'O': 1, 'P': 3,
                  'Q': 10, 'R': 1, 'S': 1, 'T': 1, 'U': 1, 'V': 4, 'W': 4, 'X': 8,
                  'Y': 4, 'Z': 10}

R_VAL = {'А': 1, 'Б': 3, 'В': 1, 'Г': 3, 'Д': 2, 'Е': 1, 'Ё': 3, 'Ж': 5,
...
if all(char in E_VAL for char in word):
    values = E_VAL
elif all(char in R_VAL for char in word):
    values = R_VAL
else:
    print("Введено недопустимое слово")
    exit()

score = sum(values[char] for char in word)
print("Стоимость Вашего слова:", score, "очков")